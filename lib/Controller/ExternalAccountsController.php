...
class ExternalAccountsController extends Controller {
	...
	/**
	 * Create a mail account through an external provider
	 *
	 * @NoAdminRequired
	 *
	 * @param string $providerId The provider ID (e.g., 'ionos')
	 * @param array<string, mixed> $parameters Provider-specific parameters
	 * @return JSONResponse
	 */
	#[TrapError]
	public function create(string $providerId, array $parameters = []): JSONResponse {
		try {
	 ...
	 */
	#[TrapError]
	public function generatePassword(string $providerId, ?int $accountId = null): JSONResponse {
		if ($accountId === null) {
