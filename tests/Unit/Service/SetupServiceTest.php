...
class SetupServiceTest extends TestCase {
	...
	private function assertAccountPropertiesMatch(
		...
		return $account->getName() === $accountName
			&& $account->getEmail() === $emailAddress
			&& $account->getInboundHost() === $imapHost
			&& $account->getInboundPort() === $imapPort
			&& $account->getInboundSslMode() === $imapSslMode
			&& $account->getInboundUser() === $imapUser
			&& $account->getOutboundHost() === $smtpHost
			&& $account->getOutboundPort() === $smtpPort
			&& $account->getOutboundSslMode() === $smtpSslMode
			&& $account->getOutboundUser() === $smtpUser
			&& $account->getUserId() === $uid
			&& $account->getAuthMethod() === $authMethod;
	}
