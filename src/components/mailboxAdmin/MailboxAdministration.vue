...
<template>
	<SettingsSection :name="t('mail', 'Email Administration')"
		...
		<div class="mailbox-administration">
			...
			<!-- Deletion modal -->
			<MailboxDeletionModal v-if="showDeleteModal"
...
<script>
import { NcEmptyContent, NcLoadingIcon, NcNoteCard, NcSettingsSection as SettingsSection } from '@nextcloud/vue'
import IconMail from 'vue-material-design-icons/Email.vue'

import MailboxListItem from './MailboxListItem.vue'
import MailboxDeletionModal from './MailboxDeletionModal.vue'
import { getMailboxes, getProviders, deleteMailbox } from '../../service/MailboxAdminService.js'
import { showError, showSuccess } from '@nextcloud/dialogs'

export default {
	name: 'MailboxAdministration',
	components: {
		SettingsSection,
		NcEmptyContent,
		NcLoadingIcon,
	...
	methods: {
		...
		async loadMailboxes() {
			...
			try {
				// For now, we'll load from the first provider
				// In a real scenario, you might want to load from all providers
		...
		handleEdit(mailbox) {
			...
			console.log('Edit mailbox', mailbox)
			showError(this.t('mail', 'Edit functionality not yet implemented'))
		},
